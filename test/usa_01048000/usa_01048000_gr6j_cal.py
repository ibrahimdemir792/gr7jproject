...
data_path = next(working_directory.rglob('**/usa_01048000.pkl'), None)
if data_path:
...
start_date = datetime.datetime(1961, 1, 1, 0, 0)
...
dir_path = next(Path.cwd().rglob('outputs_usa_01048000'), None)
if dir_path is not None:
    output_path = dir_path / 'gr6j_param_usa_01048000.json'
    with open (output_path, "w") as file:
        json.dump(parameters, file)

start_date = datetime.datetime(1948, 1, 1, 0, 0)
end_date = datetime.datetime(1960, 12, 31, 0, 0)
mask = (df['date'] >= start_date) & (df['date'] <= end_date)
...
filtered_input = validation_data[validation_data.index >= datetime.datetime(1949, 1, 1, 0, 0)]
filtered_output = outputs[outputs.index >= datetime.datetime(1949, 1, 1, 0, 0)]
