...
data_path = next(working_directory.rglob('**/adana_data.pkl'), None)
if data_path:
...
end_date = datetime.datetime(2015, 12, 30, 0, 0)
...
dir_path = next(Path.cwd().rglob('outputs_adana'), None)
if dir_path is not None:
    output_path = dir_path / 'gr6j_param_adana.json'
...
filtered_input = validation_data[validation_data.index >= datetime.datetime(1999, 1, 1, 0, 0)]
filtered_output = outputs[outputs.index >= datetime.datetime(1999, 1, 1, 0, 0)]
