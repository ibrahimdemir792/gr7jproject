...
data_path = next(working_directory.rglob('**/L0123003.pkl'), None)
if data_path:
...
start_date = datetime.datetime(1992, 1, 1, 0, 0)
end_date = datetime.datetime(2012, 12, 31, 0, 0)
...
dir_path = next(Path.cwd().rglob('outputs_L0123003'), None)
if dir_path is not None:
    output_path = dir_path / 'gr7j_param_L0123003.json'
...
end_date = datetime.datetime(1991, 12, 31, 0, 0)
mask = (df['date'] >= start_date) & (df['date'] <= end_date)
...
filtered_input = validation_data[validation_data.index >= datetime.datetime(1985, 1, 1, 0, 0)]
filtered_output = outputs[outputs.index >= datetime.datetime(1985, 1, 1, 0, 0)]
